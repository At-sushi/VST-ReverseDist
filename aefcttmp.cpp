...
{
	// BlockSizeからleast以上の最小約数を検知
	auto getDivisor = [](int blockSize, int least)
	{
		const std::function<int(int)> invert = [blockSize](int x){ return blockSize / x; };		// blockSizeに対しての逆数

		const auto range = boost::join(boost::irange(least, (int)sqrtf(blockSize) + 1),
			boost::irange(least - 1, 0, -1) | boost::adaptors::transformed(invert));			// 約数検知する範囲
		for (int i : range)
			if (blockSize % i == 0)
				return i;
